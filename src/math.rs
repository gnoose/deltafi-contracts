...
/// calculate div ceil
pub fn div_ceil(value0: U256, value1: U256) -> Option<U256> {
...
/// calculate div floor
pub fn div_floor(value0: U256, value1: U256) -> Option<U256> {
    value0.checked_div(value1)
}

/// calculate mul ceil
pub fn mul_ceil(value0: U256, value1: U256) -> Option<U256> {
    value0.checked_mul(value1)
}

/// calculate mul floor
pub fn mul_floor(value0: U256, value1: U256) -> Option<U256> {
    value0.checked_mul(value1)
}
...
pub fn solve_quadratic_function_for_target(v1: U256, k: U256, fair_amount: U256) -> Option<U256> {
    // V0 = V1+V1*(sqrt-1)/2k
...
mod tests {
    ...
    fn test_integer_sqrt() {
        ...
        assert_eq!(integer_sqrt(value * value).unwrap(), value);
    }
    ...
    fn test_mul_ceil() {
        ...
        assert_eq!(mul_ceil(value, value).unwrap(), value * value);
    }
    ...
    fn test_mul_floor() {
        ...
        assert_eq!(mul_floor(value, value).unwrap(), value * value);
    }
    ...
    fn test_general_integrate() {
        ...
        assert_eq!(general_integrate(v0, v1, v2, i, k).unwrap(), expected);
    }
    ...
    fn test_solve_quadratic_function_for_trade() {
        ...
        let mut square_root = U256::one()
            ...
            .unwrap();
        square_root =
            integer_sqrt(b.checked_mul(b).unwrap().checked_add(square_root).unwrap()).unwrap();
    ...
    fn test_solve_quadratic_function_for_target() {
        ...
        let mut sqrt = div_ceil(
            k.checked_mul(fair_amount)
                .unwrap()
                .checked_mul(4.into())
                .unwrap(),
            v1,
        ...
        .unwrap();
        sqrt = integer_sqrt(
            sqrt.checked_add(U256::one())
                .unwrap()
                .checked_mul(U256::one())
                .unwrap(),
        )
        .unwrap();

        let premium = div_ceil(
            sqrt.checked_sub(U256::one()).unwrap(),
            k.checked_mul(2.into()).unwrap(),
        )
        .unwrap();

        // V0 is greater than or equal to V1 according to the solution
        let expected = v1
            .checked_mul(premium.checked_add(U256::one()).unwrap())
            .unwrap();
